      ...
      <section className="onboarding-grid" id="crm-new-client">
        <article className="quick-card">
          ...
          <form onSubmit={submitForm("cliente")}>
            <label><span>Nombres *</span><input name="nombres" placeholder="Ej. María José" required /></label>
            <label><span>Apellidos</span><input name="apellidos" placeholder="Ej. López Ruiz" /></label>
            <label><span>Correo electrónico</span><input name="email" type="email" placeholder="cliente@correo.com" /></label>
            <label><span>Teléfono / WhatsApp</span><input name="telefono" inputMode="tel" placeholder="+505 8888 8888" /></label>
            <label className="wide"><span>Dirección física</span><input name="direccion" placeholder="Barrio, calle y referencia (no escribas aquí el correo)" /></label>
            <label className="wide"><span>Estado comercial</span><select name="estado" defaultValue="activo"><option value="activo">Activo</option><option value="prospecto">Prospecto</option><option value="inactivo">Inactivo</option><option value="suspendido">Suspendido</option></select></label>
            <fieldset className="wide notification-preferences"><legend>Avisos transaccionales</legend><label><input name="notificaciones_email" type="checkbox" defaultChecked /> Correo</label><label><input name="notificaciones_whatsapp" type="checkbox" defaultChecked /> WhatsApp</label></fieldset>
      ...
      {selectedClient ? <aside className="drawer" aria-label="Ficha del cliente">
        ...
        {editing ? (
          <form className="edit-form" onSubmit={(event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget).entries()); mutation.mutate({ action: "editar_cliente", values: { ...values, id: selectedClient.id } }, { onSuccess: () => setEditing(false) }); }}>
            <p className="client-code">Código de cliente <b>{selectedClient.codigo || "Se asignará al guardar"}</b></p>
            <label><span>Nombres *</span><input name="nombres" defaultValue={selectedClient.nombres || ""} required /></label>
            <label><span>Apellidos</span><input name="apellidos" defaultValue={selectedClient.apellidos || ""} placeholder="Apellidos" /></label>
            <label><span>Correo electrónico</span><input name="email" type="email" defaultValue={selectedClient.email || ""} placeholder="cliente@correo.com" /></label>
            <label><span>Teléfono / WhatsApp</span><input name="telefono" inputMode="tel" defaultValue={selectedClient.telefono || ""} placeholder="+505 8888 8888" /></label>
            <label className="wide"><span>Dirección física</span><input name="direccion" defaultValue={selectedClient.direccion || ""} placeholder="Barrio, calle y referencia" /></label>
            <label className="wide"><span>Estado comercial</span><select name="estado" defaultValue={selectedClient.estado_cliente || "activo"}><option value="activo">Activo</option><option value="prospecto">Prospecto</option><option value="inactivo">Inactivo</option><option value="suspendido">Suspendido</option></select></label>
            <fieldset className="wide notification-preferences"><legend>Avisos transaccionales</legend><label><input name="notificaciones_email" type="checkbox" defaultChecked={selectedClient.notificaciones_email !== false} /> Correo</label><label><input name="notificaciones_whatsapp" type="checkbox" defaultChecked={selectedClient.notificaciones_whatsapp !== false} /> WhatsApp</label></fieldset>
...
const Container = styled.section`
  ...
  .onboarding-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.quick-card{padding:18px}.quick-card header{display:flex;gap:11px;margin-bottom:14px}.quick-card header>span{display:grid;place-items:center;width:36px;height:36px;border-radius:10px;background:#eff6ff;color:#0284c7}.quick-card form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.quick-card form label,.edit-form label{display:grid;gap:6px;min-width:0}.quick-card form label>span,.edit-form label>span{color:${({ theme }) => theme.colorSubtitle};font-size:11px;font-weight:850;letter-spacing:.025em}.quick-card form .wide,.quick-card form button{grid-column:span 2}.quick-card input,.quick-card select,.filters input,.filters select,.edit-form input,.edit-form select{width:100%;box-sizing:border-box;min-width:0;border:1px solid ${({ theme }) => theme.color2};border-radius:10px;background:${({ theme }) => theme.bgtotal};color:${({ theme }) => theme.text};padding:11px}.quick-card input:focus,.quick-card select:focus,.edit-form input:focus,.edit-form select:focus{outline:3px solid rgba(14,165,233,.16);border-color:#0ea5e9}.pending-invitations{display:grid;gap:5px;margin-top:11px;font-size:12px;color:${({ theme }) => theme.colorSubtitle}}
  .quick-card form .notification-preferences,.edit-form .notification-preferences{grid-column:1 / -1;display:flex!important;flex-wrap:wrap;align-items:center;gap:8px 18px;min-width:0;margin:2px 0 0;padding:11px 13px;border:1px solid ${({ theme }) => theme.color2};border-radius:11px;color:${({ theme }) => theme.colorSubtitle};}
  .notification-preferences legend{padding:0 5px;font-size:11px;font-weight:850;letter-spacing:.025em}.notification-preferences label{display:inline-flex!important;align-items:center;gap:7px!important;width:auto!important;margin:0;font-size:12px;font-weight:750}.notification-preferences input{width:16px!important;height:16px;margin:0;padding:0!important;accent-color:${v.colorPrincipal}}
