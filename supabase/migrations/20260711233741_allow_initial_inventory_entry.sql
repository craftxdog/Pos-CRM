create or replace function public.registrar_movimiento_inventario(
  _id_empresa bigint, _id_almacen bigint, _id_producto bigint, _id_stock bigint,
  _tipo_movimiento text, _cantidad numeric, _fecha timestamp with time zone,
  _detalle text, _origen text, _creado_por bigint,
  _precio_compra numeric, _precio_venta numeric
)
returns bigint language plpgsql security invoker set search_path = public as $$
declare
  movimiento_id bigint;
  stock_actual numeric;
  stock_id bigint := _id_stock;
begin
  if _cantidad is null or _cantidad <= 0 then raise exception 'La cantidad debe ser mayor que cero'; end if;
  if _tipo_movimiento not in ('ingreso', 'salida') then raise exception 'Tipo de movimiento inválido'; end if;
  if _id_empresa <> (select private.current_empresa_id()) then
    raise exception 'La empresa no corresponde a la sesión';
  end if;

  select id, stock into stock_id, stock_actual
  from public.almacenes
  where id_almacen = _id_almacen and id_producto = _id_producto
  for update;

  if not found then
    if _tipo_movimiento = 'salida' then raise exception 'No existe stock disponible para este producto'; end if;
    insert into public.almacenes (id_almacen, id_producto, stock, stock_minimo)
    values (_id_almacen, _id_producto, 0, 0)
    returning id, stock into stock_id, stock_actual;
  end if;
  ...
  update public.productos set precio_compra = _precio_compra, precio_venta = _precio_venta
